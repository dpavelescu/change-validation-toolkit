...
**One line:** the toolkit's real footprint is **①②** — it *runs* those. Everything else it leaves to you; for external NFRs it offers *opt‑in* record‑keeping (a coverage flag + an audit trace), never a relay in the path.
...
6. **Test‑level discipline (avoids brittle tests)** — evidence is specified at the **lowest sufficient level**; integration/e2e are justified by what only they prove, not used by default; the brittleness/cost of over‑using high‑level tests is explicitly controlled.
