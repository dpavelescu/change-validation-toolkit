...
2. **Disposition justification** — every proposed `change`/`remove` traces to a **criteria delta** (an AC `moved`/`retired` in the Criteria IDs), never to a test result. A result‑driven disposition is a sign of masking a regression — flag it. — *uses* **validation‑plan**.
