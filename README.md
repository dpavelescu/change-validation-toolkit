...
> **Status — Foundation + Phase 2 + Phase 3 complete, Copilot build.** Built: the **Testing Strategy**, the **derived Validation Rules**, the **Source‑Map Manifest**, **Change Classification**, the tool‑managed **Criteria IDs**, the per‑change **Validation Plan**, the **Behavior Baseline** (the behavior snapshot that makes the honesty rule checkable), the **Execution Runner** (drives the project's *own* suite to observe behavior — the toolkit's first running piece), **Test Reconciliation** (the toolkit *specifies* what each witness asserts from the criteria and *verifies* the result; **authoring is handed off**, like production code), the **Correction Loop** (drives a failing change to green by emitting structured handoffs — `test-request`s and `fix-request`s — for whoever implements; **the toolkit writes no code**), and the **Evidence Ledger** (the durable audit trail of what was validated and why). The model is **specified** end-to-end; the parallel `.claude/` build follows.
...
.github/                            ← GitHub Copilot build
  agents/
    ...
    review-plan        gate the plan: coverage + fate justification (Phase 2)
    ...
    specify-tests                 own what each witness asserts (from criteria) + verify; authoring handed off (Phase 3)
