...
The Validation Plan (criteria, gates), the Behavior Baseline reconciliation (preserved/justified), the run records (green evidence per test), the test‑reconciliation record (dispositions + justifications), and any decisions/limitations raised. Apply the **evidence‑ledger** skill.
...
A durable per‑change record: `validated[]` (criterion → test → evidence), `behavior-preserved[]`, `test-changes[]` (with justifications), `decisions[]`, `limitations[]`, `verdict` (green only on complete evidence).
