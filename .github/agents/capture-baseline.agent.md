...
**Args:** `change=<diff|branch|PR>` · `classification=<path>` (blast radius + change‑types) · `plan=<path>` (the `behavior-preservation` track — the surfaces to pin) · `ledger=<path>` (criteria deltas) · `baseline=<path>` (default `.validation/<change>/baseline.md`).
