...
- **Machine artifact** *(for `specify-tests` and the correction loop)* — the **Behavior Baseline** (pinned behavior per the **behavior‑baseline** schema) and its **reconciliation record**: `justified[]` (each naming the AC it maps to and the plan disposition it confirms) · `regressions[]` (fed back into the loop).
- **Human** *(only where a person must act)* — each **`boundary-decision`** as a structured question and each **limitation** as a gap to close, in the shape of the **escalation** skill: the question or gap, the context forcing it, the owning authority (or what would close it), and what it blocks. Lead with the decision or gap — no narration of what was pinned.
