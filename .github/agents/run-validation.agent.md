...
The blast radius (which surfaces), the Validation Plan's `local-gate`/`ci-gate` (what evidence to run), and the execution commands resolved from the Source‑Map (`build-commands`, parity‑checked against `ci-config`; the `tests` kind for selectors; the **`test-report`** kind for results). Apply the **execution‑runner** skill. *(This agent executes the project's suite — the toolkit's first running piece. Results come from the machine‑readable report, not the console; `gate=ci` runs within the pipeline, not dispatched by the toolkit.)*

## Process (preflight → resolve → scope → run → check → record)
1. **Preflight capabilities** — verify the capability/environment each in‑scope category `needs` (Source‑Map) is present; an absent prerequisite → the category is **out of scope for this run**, a **limitation up front** (no perfect unrunnable test). Then **Resolve** the commands from the Source‑Map `build-commands` (parity‑checked vs `ci-config`). A critical unresolvable command → blocking **limitation**; never guess the invocation.
