...
The Validation Plan (per‑AC fates, witnesses, gates), the Criteria Identity (the assertion authority + `moved`/`retired` deltas), and the Behavior Baseline (baseline provenance + which behavior deltas are `justified`). Apply the **test‑reconciliation** and **escalation** skills (a delete recommendation is a structured `decision`). Discover existing witnesses **read‑only** via the Source‑Map `tests` kind.

## Process (confirm fate → materialize → evidence → record)
1. **Confirm fates** — per active AC, resolve the plan's provisional fate against the Criteria Identity delta and the baseline sort: `add` (new), `change` (AC `moved`), `keep` (unchanged), `remove` (AC `retired`). Then the plan's **behavior‑preservation** track: for each blast‑radius surface no AC owns with `none-yet`, `add` a **regression witness** from the baseline. A `change`/`remove` with **no** criteria delta → **blocked** (regression‑laundering): route the behavior delta to a regression (loop input) or a **decision** (criteria gap), never a silent edit.
2. **Materialize** — author/adjust the witness from its **provenance** (criterion witness → the AC; regression witness → the baseline), and **stamp its `AC-N` tag from the Criteria Identity** (regression witness → its `surface-id`) so witness→criterion is traceable. Never assert from the new impl; read impl for wiring only, flagged. An **implementation‑coupled** existing test (brittle: red while behavior `preserved`) is **driven to resolution, not just flagged**: if it maps to a real observable behavior → **`repair`** it (decouple from internals; assert the **behavior** from the baseline; re‑tag to the owning criterion if any) — justified by the `preserved` verdict, **no criteria delta required** (the one exception to the honesty lock); if it guards **no** observable behavior → emit a **`remove` recommendation** as a structured `decision` (per the **escalation** skill — test ref · what it asserts · why low‑relevance · recommended delete), human‑approved, never a silent delete. An un‑observable AC → **criteria gap → decision**; a surface you can't invoke/build → **limitation**.
