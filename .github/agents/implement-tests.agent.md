...
2. **Materialize** — author/adjust the witness from its **provenance** (criterion witness → the AC; regression witness → the baseline). Never assert from the new impl; read impl for wiring only, flagged. An un‑observable AC → **criteria gap → decision**; a surface you can't invoke/build → **limitation**.
