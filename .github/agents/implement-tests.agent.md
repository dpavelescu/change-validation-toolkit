...
2. **Materialize** — author/adjust the witness from its **provenance** (criterion witness → the AC; regression witness → the baseline), and **stamp its `AC-N` tag from the Criteria Identity** (regression witness → its `surface-id`) so witness→criterion is traceable. Never assert from the new impl; read impl for wiring only, flagged. An un‑observable AC → **criteria gap → decision**; a surface you can't invoke/build → **limitation**.
