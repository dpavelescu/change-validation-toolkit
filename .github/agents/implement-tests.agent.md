...
The Validation Plan (per‑AC fates, witnesses, gates), the Criteria Identity (the assertion authority + `moved`/`retired` deltas), and the Behavior Baseline (baseline provenance + which behavior deltas are `justified`). Apply the **test‑reconciliation** skill. Discover existing witnesses **read‑only** via the Source‑Map `tests` kind.
...
2. **Materialize** — author/adjust the witness from its **provenance** (criterion witness → the AC; regression witness → the baseline), and **stamp its `AC-N` tag from the Criteria Identity** (regression witness → its `surface-id`) so witness→criterion is traceable. Never assert from the new impl; read impl for wiring only, flagged. A **`repair`** request from the correction loop (a *brittle* red test, behavior `preserved`) → **decouple** the witness from the changed internals **without altering the asserted behavior** — justified by the baseline's `preserved` verdict, **no criteria delta required** (the one exception to the honesty lock). An un‑observable AC → **criteria gap → decision**; a surface you can't invoke/build → **limitation**.
