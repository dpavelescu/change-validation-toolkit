...
## Guards
Criteria‑gate (no criteria → Not ready) · advisory (no run/edit) · provisional‑fates · fate‑justification (change/remove ↔ criteria delta, never a result) · coverage (every active AC witnessed or `none-yet`) · sufficiency (evidence *proves* each AC; uncovered need → Strategy gap) · blast‑radius regression coverage (AC‑owned, witnessed, or `out-of-scope`) · lowest‑sufficient‑level (integration/e2e justified, not default) · gate placement (local‑first, fail‑fast; CI‑only is expected) · non‑automatable honesty (admit + name runtime witness) · source (critical unretrievable → *limitation*) · authority (vs the normative owner; contract change → decision) · coverage‑alignment (re‑align by default; delete only guards‑nothing) · testability (un‑observable → decision) · decision‑escalation (ambiguity/contradiction/ownership/contract) · humans approve.
