...
## Inputs
The change, the story, the generated **Validation Rules**, the **Source‑Map Manifest**; the delegated sub‑agents apply their own skills. *(Delegation uses your Copilot's subagent tool (`agent`) — ensure it's enabled; pass each delegate only its slice.)*
