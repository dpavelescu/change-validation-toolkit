...
## Output (classify per output-style)
The Validation Plan is **one artifact serving both audiences** (the *sections-for-both* pattern — not a duplicate report):
- **As a schema** *(the executor replays it)* — per the **validation‑plan** schema: criteria track · behavior‑preservation track · `coverage-alignment` · local/CI gates · dispositions marked provisional.
- **For the approver** *(the same artifact, led by a concise "what to approve")* — per‑AC coverage at a glance, the proposed dispositions and the criteria delta each traces to, surfaces covered vs. `out-of-scope`, and any open decisions blocking sign‑off. The schema's tracks are its named sections; no separate prose copy.

Or **Not ready** — a resumable agenda naming exactly what's missing (the reviewer's findings), no plan written. Any `decision`/`limitation` follows the structured shape of the **escalation** skill (a decision: its question, the context forcing it, the owning authority, what it blocks).
