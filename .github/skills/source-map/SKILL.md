...
- **Capability prerequisites are declared, then preflighted** — a test category that needs a specialized capability/environment to run (e.g. `performance`/`load` need a load generator + perf env; `failure-resilience` needs fault‑injection tooling + a soak/staging env) records that prerequisite (a `needs:` note on its `tests` entry). The Execution Runner **preflights** it: a category whose capability isn't present is **out of scope for this run** (a *limitation* raised up front), never a perfect test that can't be executed. Categories with no special need (`unit`/`component`/`contract`) preflight trivially.
