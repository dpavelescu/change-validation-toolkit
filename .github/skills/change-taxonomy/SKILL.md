...
1. **Coverage map** (`test → lines`) — map the diff's lines to the tests that execute them. The toolkit **generates it by invoking your runner in coverage mode** (a variant of the normal run). It needs **per‑test** coverage (test‑impact / per‑test‑coverage tooling, which many runners support); where only **aggregate** line coverage exists, that still tells which lines are covered *at all* (so an uncovered changed line is a flagged gap) — for the `test → line` link it then falls back to (2) and widens. Because per‑test coverage is recorded from real execution it captures **dynamic** wiring (DI, reflection, event/queue hops) static analysis can't see — the strongest signal behind the headline case **where the project has per‑test‑coverage tooling**; without it the toolkit falls back to (2) and widens, so treat this as the best‑case path, not the default everywhere.
...
4. **System / e2e** — caught by **flow participation**: which flow a test exercises comes from a coverage run over the e2e suite if available, else from the test's location + its scenario/name + the surfaces it sets up. A changed surface that any such flow touches pulls that test in. When this can't be established, **widen** (run the e2e suite for the touched surfaces) rather than guess.

**Safe failure mode — widen, never silently miss.** **Signal is weak** when a surface has no per‑test coverage, or the change crosses a boundary static analysis can't follow (DI, reflection, events/queues, HTTP), or it touches framework wiring/config. Then the scope **widens** — to the enclosing module, the surface's own test file(s), and the e2e flows that touch it — and the gap is **flagged**. An over‑run costs runtime; an under‑run risks a false green, so the bias is always to widen. Determinism is bounded by the project's instrumentation, not by guessing: with the code, the tests, and a per‑test coverage run, the link is precise for everything the tests execute — and only as good as that coverage where it's missing, which is why the bias is to widen and flag.
