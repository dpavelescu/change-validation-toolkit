...
**Safe failure mode — widen, never silently miss.** **Signal is weak** when a surface has no per‑test coverage, or the change crosses a boundary static analysis can't follow (DI, reflection, events/queues, HTTP), or it touches framework wiring/config. Then the scope **widens** — to the enclosing module, the surface's own test file(s), and the e2e flows that touch it — and the gap is **flagged**. An over‑run costs runtime; an under‑run risks a false green, so the bias is always to widen. Determinism is bounded by the project's instrumentation, not by guessing: with the code, the tests, and a per‑test coverage run, the link is exact for everything the tests execute.
