...
- **Idiomatic & data‑backed** — witnesses are authored in the project's own **test‑specification style** (xUnit, **BDD/Gherkin scenarios**, property/table‑based) and frameworks (learned from the typed `tests` exemplars + `coding-guidelines`), with fixtures from `test-data`; a style/framework it can't author or un‑buildable test data is a *limitation*, surfaced, never faked.
