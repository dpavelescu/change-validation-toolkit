...
- **Level & gate placement.** Each evidence item names its **`level`** at the **lowest category that proves it** — integration/e2e only where a lower level can't (avoids brittle tests). The `local-gate`/`ci-gate` split then follows level + runnability: fast low‑level tests (unit/component/contract) go in `local-gate` to **fail fast**; cross‑boundary / infra‑dependent tests (integration, e2e) that can't run on a dev machine go in `ci-gate`. CI‑only by necessity is **expected placement, not a gap**.
