...
The Execution Runner is the toolkit's **execution substrate** — the first piece that *runs* anything (everything before it proposes, never runs or edits). **Running is the toolkit's own step, not a handoff:** it writes no code (authoring is external), but it executes the suite itself — by invoking your **own test runner** (the standard command devs and CI already use — never a parallel harness the toolkit invents) over the change's blast‑radius slice, returning **structured observations**: what ran, what each result evidences about behavior, and whether it reproduces. It **runs but never edits** — observation only; deciding what to *do* with a result belongs to the Behavior Baseline (pin/reconcile) and the correction loop. Per‑change, **recorded in‑repo and committed** so CI replays the same scope and commands (local↔CI parity).
...
- **Clean fail** — the test ran and asserted false → **fed back into the loop**, a behavior signal. Never escalated to a human.
...
A red test and a broken harness look identical at a glance; the runner must not conflate them — it sorts every non‑pass into **signal or gap**.
...
3. **Scope & order (fail‑fast)** to the blast radius and the relevant gate — run the slice via selective invocation, **cheapest first**: the `local-gate` (fast low‑level tests) locally **before** the `ci-gate` (cross‑boundary / infra tests), so problems surface early and a local failure short‑circuits the CI‑level run. Scope is **minimal on trustworthy signal but widens when the signal is weak** — an over‑run costs time, an under‑run risks a false green. A test that legitimately can't run locally (CI‑only by placement) is **deferred to its gate, not a limitation** — a limitation is only a test that can't run *where it's supposed to*.
4. **Run & observe** — execute, then read the **machine‑readable test report** (Source‑Map `test-report`), **not** the console; normalize it (JUnit XML / TAP / native JSON) into per‑case `outcome` + the behavior it `evidenced`. Each report case **binds to a surface by its `AC-N` tag and its `test-ref`** (recorded in the plan / baseline) — not by re‑parsing the test name. Sort each non‑pass: **clean fail → fed back into the loop**, **can't‑run → limitation**. No machine‑readable report → a **limitation** (configure a reporter), never stdout scraping. The runner **does not edit** tests or implementation.
5. **Flakiness check** — re‑run the slice (once by default; `N` per config); if any run disagrees → `flaky` (**quarantine** the surface + raise a **limitation**). Agreement is a smoke‑check, not proof of determinism (it only catches frequent flakes). Flaky behavior is never passed to the baseline as truth — you cannot evidence what you cannot reproduce.
...
- **Clean fail ≠ can't‑run** — a red test is **fed back into the loop**; a broken harness is a **limitation**.
- **No handoff for a fail** — a failing test is fed back into the loop, never escalated to a human.
...
- **Determinism or quarantine** — flaky → limitation, never baselined; re‑run agreement is a smoke‑check, not a proof of determinism.
